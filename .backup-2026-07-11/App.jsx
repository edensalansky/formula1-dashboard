...
// ECG-ish bar height per column (heartbeat pattern)
function ecgHeight(x) {
  const b = ((Math.round(x) % 12) + 12) % 12
  let h = b === 5 ? 9 : b === 4 ? 4 : b === 6 ? 3 : b === 0 ? 2 : 1
  h += Math.round(1.2 * (0.5 + 0.5 * Math.sin(x * 0.045)))
  return Math.max(1, Math.min(HR_ROWS, h))
}
const HR_HEIGHTS = Array.from({ length: HR_COLS }, (_, c) => ecgHeight(c))
const HR_HIGH = 9 // column height at/above this = a heart-rate spike — flag it red
const SIM_SPEED = 4 // fake acceleration: 1 real second = 4 sim seconds (60-min session ≈ 15 real min)
...
function TrainingScreen({ onNav }) {
  ...
  const drawHR = (el) => {
    ...
    let reveal = 0
    for (let c = 0; c < fullCols && c < HR_COLS; c++) reveal += HR_HEIGHTS[c]
    if (fullCols < HR_COLS) reveal += Math.floor((curMin - fullCols) * HR_HEIGHTS[fullCols])
    let left = started ? reveal : 0
    for (let c = 0; c < HR_COLS; c++) {
      const litThisCol = Math.max(0, Math.min(left, HR_HEIGHTS[c]))
      const isHigh = HR_HEIGHTS[c] >= HR_HIGH // heart-rate spike — flag so the coach knows to stop
  ...
  // ===================== SESSION REVIEW (after FINISH) =====================
  if (finished) {
    const finalMin = Math.min(elapsedRef.current / 60, RULER_MAX)
    const sysDone = SYS_EVENTS.filter((e) => e.start <= finalMin)
    const cols = Math.max(1, Math.round(finalMin))
    const meanH = HR_HEIGHTS.slice(0, cols).reduce((a, b) => a + b, 0) / cols
    const avgHR = Math.round(118 + meanH * 16)

    return (
      <>
        <img className="logo" src={`${A}/c1logo.svg`} alt="C1" />
        <div className="brand">SESSION REVIEW</div>
        <Nav active="training" onNav={onNav} />
        <div className="sh-avatar">SH</div>

        <h1 className="sim-title">Session Review</h1>
        <div className="sum-sub">Simulator Mode · {fmtClock(elapsed)} logged</div>

        <div className="sum-panel">
          <button className="sim-btn sum-restart" onClick={restart}>NEW SESSION</button>

          <div className="sum-stats">
            <div className="sum-tile"><b>{fmtClock(elapsed).slice(3)}</b><span>Duration</span></div>
            <div className="sum-tile"><b>{sysDone.length}</b><span>System alerts</span></div>
            ...
            <div className="sumtl-viewport" ref={reviewViewportRef}>
              <div className="sumtl-track" style={{ width: zoom * 100 + '%' }}>
                ...
                {allEvents.map((e) => (
                  ...
                  >
                    {e.type === 'user' && <span className="notif__sh">SH</span>}
                    {e.label}
                  </div>
                ))}
  ...
  return (
    <>
      ...
      <div className="sim-panel">
        ...
        <div className="sim-min">
          ...
          <div className="sim-viewport" ref={viewportRef} onScroll={(e) => setScrollX(e.currentTarget.scrollLeft)}>
            <div className="sim-track" style={{ width: zoom * 100 + '%' }} onClick={onTrackClick}>
              {/* fine per-second grid (emerges as you zoom in) */}
              {zoom > 2 && <div className="sec-grid" />}
              {/* once zoomed in, the ruler matches the review exactly: a fine
                  HH:MM:SS mark every 30s. Zoomed out, it's the sparse 10/5-min ruler. */}
              {zoomed ? (
                <>
                  {Array.from({ length: Math.floor(RULER_MAX / 0.5) + 1 }).map((_, i) => {
                    const m = i * 0.5
              ...
              {notes.map((n, i) =>
                zoomed ? (
                  ...
                  >
                    <span className="notif__sh">SH</span>
                    {n.text}
                  </div>
                ) : (
                  <div
        ...
        {/* FINISH — open the session review */}
        <button
